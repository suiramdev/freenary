...
export function useRevision(data: unknown, token: number) {
  const [prev, setPrev] = useState({ data, revision: 0, token });
  if (prev.data !== data || prev.token !== token) {
    const next = { data, revision: prev.revision + 1, token };
    setPrev(next);
...
export function useChartController({
  ...
  const [entrance, setEntrance] = useState({ done: !animate, revision });
  if (entrance.revision !== revision) {
    setEntrance({ done: !animate, revision });
  }
  const entranceDone =
  ...
  const markEntranceDone = useCallback(
    () => setEntrance({ done: true, revision }),
    [revision]
  );
