...
import type { CategoryParentOption } from "./category-parent-select";
...
const effectiveCategoryColor = (
  values: CustomCategoryValues,
  parentOptions: CategoryParentOption[]
): CategoryColor => {
  if (values.parentKey === null) {
    return values.color;
  }

  const parent = parentOptions.find(
    (option) => option.key === values.parentKey
  );

  return parent?.appearance.color ?? values.color;
};
...
export const CustomCategoryDialog = ({
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className={TALLER_THAN_A_SHORT_VIEWPORT} size="lg">
        <form
          onSubmit={(event) => {
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              {m.settings_category_dialog_description()}
            </DialogDescription>
          </DialogHeader>
          ...
          <FieldGroup>
            ...
            <form.Subscribe selector={(state) => state.values.parentKey}>
              {(parentKey) =>
                parentKey === null ? (
            ...
            <form.Field name="icon">
              {(field) => (
                <FieldSet>
                  ...
                  <form.Subscribe
                    selector={(state) =>
                      effectiveCategoryColor(state.values, parentOptions)
                    }
                  >
                    {(color) => (
                      <ToggleGroup
