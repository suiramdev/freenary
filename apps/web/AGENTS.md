...
- Env vars must be prefixed `VITE_` and declared in `@freenary/env/src/web.ts`. One is set by the image build, not by an operator: `VITE_FREENARY_VERSION` carries the release tag (`FREENARY_VERSION` build argument in `apps/web/Dockerfile`), and `@/lib/docs.ts` turns it into the documentation link the account menu opens.
