...
- Charts composed by the model, proactive insights and an in-browser local tier are all absent. The local tier in particular waits on function calling in WebLLM, which is still marked work-in-progress upstream; shipping it as a silently weaker tool loop would be worse than not shipping it.
