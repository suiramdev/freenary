...
- When adding a new variable: add the Zod schema here, document it in `apps/server/.env` (or `apps/web/.env`), and update `docker-compose.yml` if it affects container networking.
