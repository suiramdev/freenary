...
} from "@freenary/ui/components/fluid-hover-highlight";
import { ScrollArea } from "@freenary/ui/components/scroll-area";
import {
  TabsSubtle,
  TabsSubtleItem,
} from "@freenary/ui/components/tabs-subtle";
import {
  useFluidHover,
    ...
    const move = useCallback(
      (to: 1 | -1 | "first" | "last") => {
        ...
        if (to === "first") next = enabled[0];
          ...
          if (pos === -1)
            next = to === 1 ? enabled[0] : enabled[enabled.length - 1];
...
export interface CommandMenuTabsProps extends Omit<
  HTMLAttributes<HTMLDivElement>,
  "onSelect"
> {
  tabs: readonly CommandMenuTab[];
    ...
    return (
      ...
      >
        <div
          ...
          className="scrollbar-hide flex min-w-0 flex-1 items-center overflow-x-auto"
        >
          <TabsSubtle
            size="compact"
      ...
      >
        {caps.map((cap, i) => (
          <span
            key={`${cap}-${i}`}
            className={cn(
              "bg-hover text-muted-foreground flex items-center justify-center rounded-[5px]",
              compact
  ...
  ({ hints, className, children, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          "text-muted-foreground flex shrink-0 items-center overflow-hidden",
          compact
