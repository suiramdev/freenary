...
import { useShape, shapeMap } from "@freenary/ui/lib/shape-context";
import {
  SizeProvider,
  useSize,
  type SizeVariant,
...
> {
  ...
  name?: string;
  required?: boolean;
  /** The typed query, on every keystroke. A list that is filtered by a
   *  server rather than by `filter` reads it to know what to ask for. */
  onQueryChange?: (query: string) => void;
  /** The popup opening and closing, for a caller that only loads its items
...
>({
  ...
  const createItem = useMemo<ComboboxItemData | null>(() => {
    ...
    const exists = items.some(
      (item) => itemLabel(item).toLocaleLowerCase() === lower
    );
  ...
  const listItems = useMemo<readonly ComboboxItemData[]>(() => {
    const visible = hideChecked
      ? items.filter((item) => !values.includes(itemValue(item)))
      : items;
    return createItem ? [...visible, createItem] : visible;
  }, [items, hideChecked, values, createItem]);
  const allSelected =
    hideChecked && query === "" && items.length > 0 && listItems.length === 0;
  ...
  const handleValueChange = useCallback(
    (next: T[] | T | null) => {
      ...
      let nextValues = picked
        .filter((item) => !isCreateItem(item))
        .map(itemValue);
      if (created) {
      ...
      onValueChange?.(
        (isMultiple
          ? nextValues
          : (nextValues[0] ?? "")) as ComboboxValue<Multiple>
      );
    },
  ...
  const filterFn = useMemo(() => {
    ...
    return (item: ComboboxItemData, q: string) =>
      isCreateItem(item) || match(item, q);
    ...
    <ComboboxContext.Provider value={ctx}>
      <ComboboxHighlightContext.Provider value={highlight}>
        <ComboboxPrimitive.Root
          ...
          value={
            (isMultiple ? selectedItems : (selectedItems[0] ?? null)) as never
          }
          ...
          onItemHighlighted={(item, details) =>
            setHighlight(
              item === undefined
                ...
                : {
                    index: details.index,
                    keyboard: details.reason !== "pointer",
                  }
            )
          }
          disabled={disabled}
  ...
  ],
  {
    variants: {
      variant: {
        ...
        // input field.
        bordered:
          "ring-border hover:bg-muted/50 focus-within:bg-card bg-transparent",
        // Invisible at rest — the InputGroup field ladder: muted fill +
        // ring on hover, card fill when focused.
        borderless:
          "hover:bg-muted/50 hover:ring-border focus-within:bg-card focus-within:ring-border bg-transparent ring-transparent",
      },
    },
    defaultVariants: {
      variant: "bordered",
...
const chipRemoveClass = cn(fieldButtonClass, "hover:bg-active rounded");
      ...
      <div className="flex flex-col gap-1">
        ...
        >
          ...
          <ComboboxPrimitive.Input
            ...
            // The field shows the current selection, so a keystroke has to
            // replace it: without this the caret lands mid-label and typing
            // splices the query into the name already there.
                ...
                <>
                      ...
                      return (
                        ...
                        >
                          <ComboboxPrimitive.Chip
                            aria-label={label}
                            className={cn(
                              "bg-hover text-foreground inline-flex max-w-full shrink-0 items-center gap-0.5 pr-0.5 pl-2 outline-none",
                              shape.variant === "pill"
                                ? "rounded-full"
                                : "rounded-md",
                  ...
                  <span className="flex min-w-6 flex-auto">
                    <ComboboxPrimitive.Input
                      ...
                      aria-invalid={!!error || undefined}
                      className={cn(
                        "text-foreground placeholder:text-muted-foreground w-full min-w-0 rounded-none bg-transparent font-[inherit] outline-none",
...
type PositionerProps = React.ComponentProps<
...
const ComboboxContent = forwardRef<HTMLDivElement, ComboboxContentProps>(
  (
    { className, children, side = "bottom", align = "start", sideOffset = 6 },
    ref
  ) => {
...
const ComboboxList = forwardRef<HTMLDivElement, ComboboxListProps>(
  ({ className, children }, ref) => {
    const { open, values, multiple, inputValue, createRow } =
      useComboboxContext();
      ...
      const outer = requestAnimationFrame(() => {
        inner = requestAnimationFrame(() => {
          ...
          if (container) {
            ...
            const next: number[] = [];
            rows.forEach((el, i) => {
              if (values.includes(el.getAttribute("data-value") ?? ""))
                next.push(i);
            });
...
const ComboboxItem = forwardRef<HTMLDivElement, ComboboxItemProps>(
  (
    { className, children, icon: Icon, value, disabled = false, ...props },
    ref
  ) => {
  ...
  ({ className, children, allSelected, ...props }, ref) => {
    ...
    return (
      ...
      <ComboboxPrimitive.Empty
        ref={ref}
        className={cn(
          "text-muted-foreground px-3 text-center [&:not(:empty)]:py-6",
          sizeClasses.text,
          className
        )}
        {...props}
...
export {
  ...
  itemLabel as comboboxItemLabel,
  itemValue as comboboxItemValue,
};
