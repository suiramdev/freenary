...
Data artifacts are built in CI (`.github/workflows/generate-data.yml`) and published as a GitHub Release tagged `data-YYYY-MM-DD`. The workflow runs weekly and on push to `dev` or `main` when the generation scripts or the descriptor normaliser change.
...
2. `fetch-wikidata-brands.ts` — queries Wikidata SPARQL and writes `wikidata-brands.json`.
3. `build-merchant-dictionary.ts` — fetches the pinned NSI tarball, merges NSI + Wikidata + curated supplement, and writes `merchants.jsonl.gz`.

Each step degrades gracefully when its upstream API is unreachable.

## What the dictionary does not cover

A merchant no dictionary entry, keyword rule or user correction resolves is left to the configured transaction classifier (`TRANSACTION_CLASSIFIER`), and to nothing at all when none is configured. No model weights ship with this data, and no classifier is trained here.
