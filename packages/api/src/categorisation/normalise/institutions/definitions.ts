...
// ── Regex patterns ───────────────────────────────────────────────────────
// Each institution has its own label format quirks. Patterns are ordered
// longest-first within each definition to avoid partial matches.
...
/** Boursorama: lines starting with Réf: are pure bank noise. */
const BOURSO_REF_LINE_RE = /^R[ée]f\s*:\s/iu;

/** Boursorama: backslash-delimited localisation suffix (\PARIS\ FR). */
...
export const institutionDefs: readonly InstitutionDef[] = [
  {
    bics: ["BOUSFRPP"],
